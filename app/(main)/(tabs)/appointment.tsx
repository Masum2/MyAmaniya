...
export default function AppointmentTab() {
  ...
  const handleSave = async () => {
    ...
    if (result.success) setSuccessModalVisible(true);
  };

  const handleSelectValue = (item: any, index: number) => {
    switch (activeSelectType) {
      case 'provider': setProvider({ label: item.displayName, id: item.id }); break;
      case 'client': setClient({ label: item.description, id: item.id }); break;
      case 'type': setType({ label: item, index: index }); break;
      case 'method': setMethod({ label: item, index: index + 1 }); break;
      case 'status': setStatus({ label: item, index: index }); break;
    }
  ...
  return (
    <SafeAreaView className="flex-1 bg-slate-50 dark:bg-[#121212]">
      ...
      <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'} className="flex-1">
        <ScrollView 
          ...
          contentContainerStyle={{ paddingTop: 120, paddingBottom: 120, paddingHorizontal: 20 }}
        >
          {/* Date & Time */}
          <View className="mb-6">
            ...
            <View className="flex-row gap-3">
              <Pressable onPress={() => setShowStartTimePicker(true)} className="flex-1 bg-white dark:bg-slate-800 p-4 rounded-2xl border border-slate-200 dark:border-slate-700">
                <Text className="text-[10px] text-slate-500 dark:text-slate-400">START</Text>
                <Text className="text-slate-800 dark:text-white font-medium mt-1">{startTime.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</Text>
              </Pressable>
              <Pressable onPress={() => setShowEndTimePicker(true)} className="flex-1 bg-white dark:bg-slate-800 p-4 rounded-2xl border border-slate-200 dark:border-slate-700">
                <Text className="text-[10px] text-slate-500 dark:text-slate-400">END</Text>
                <Text className="text-slate-800 dark:text-white font-medium mt-1">{endTime.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</Text>
              </Pressable>
            </View>
          </View>
          ...
          <View className="mb-6">
            ...
            {dropdowns.map((item) => (
              <Pressable key={item.key} onPress={() => {
                ...
                setModalTitle(item.label); setModalData(data || []); setActiveSelectType(item.key); setModalVisible(true);
          ...
          <Pressable onPress={handleSave} className="bg-blue-900 p-5 rounded-2xl items-center shadow-lg shadow-blue-500/30">
      ...
      <Modal visible={modalVisible} transparent animationType="slide">
        ...
        <View className="absolute bottom-0 w-full bg-white dark:bg-slate-900 rounded-t-3xl max-h-[55%] p-6">
          <View className="items-center mb-4"><View className="w-12 h-1 bg-slate-300 dark:bg-slate-600 rounded-full" /></View>
      ...
      <Modal visible={successModalVisible} transparent animationType="fade">
        <View className="flex-1 bg-black/50 justify-center items-center p-6">
          <View className="bg-white dark:bg-slate-900 rounded-3xl p-8 w-full items-center">
            ...
            <Text className="text-xl font-bold text-slate-800 dark:text-white my-4">Success!</Text>
            <Pressable onPress={() => { setSuccessModalVisible(false); router.back(); }} className="bg-blue-500 w-full p-4 rounded-2xl items-center"><Text className="text-white font-bold">Done</Text></Pressable>
          </View>
        </View>
      </Modal>
    </SafeAreaView>
